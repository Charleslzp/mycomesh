...
"""Settlement V8 request-key authorization and receipt helpers."""
...
RECEIPT_TYPE = (
    "UsageReceipt(bytes32 authorizationHash,bytes32 responseHash,address provider,address providerSigner,address relay,address pool,"
...
SETTLE_SIGNATURE = (
    ...
    "(bytes32,bytes32,address,address,address,address,uint256,uint256,uint256),bytes,bytes,bytes))"
)
BATCH_SIGNATURE = (
    "settleSignedBatch(((bytes32,bytes32,address,address,address,bytes32,uint64,bytes32,uint256,uint64,uint64),"
    "(bytes32,bytes32,address,address,address,address,uint256,uint256,uint256),bytes,bytes,bytes)[])"
)
ARTIFACT = "out/MycoSettlementV8.sol/MycoSettlementV8.json"
DEFAULT_DEPLOYMENT = "deployments/sepolia-myco-v8.json"
DEFAULT_MYCO_V8_DEPLOYMENT_PATH = DEFAULT_DEPLOYMENT
AUTH_SCHEMA = "mycomesh.x402.myco-credit-v2"
PROVIDER_SCHEMA = "mycomesh.settlement.v8.provider.v1"
SIGNED_SCHEMA = "mycomesh.settlement.v8.signed.v1"
...
def build_authorization(
    *,
    ...
    if authorization.request_id == "0x" + "0" * 64:
        raise ChainError("V8 request_id cannot be zero")
    if authorization.request_hash == "0x" + "0" * 64:
        raise ChainError("V8 request_hash cannot be zero")
    if authorization.channel == "0x" + "0" * 64:
        raise ChainError("V8 channel cannot be zero")
    if authorization.pricing_hash == "0x" + "0" * 64:
        raise ChainError("V8 pricing_hash cannot be zero")
    if authorization.deadline <= authorization.issued_at or authorization.deadline - authorization.issued_at > MAX_AUTHORIZATION_TTL:
        raise ChainError("V8 authorization lifetime must be between 1 and 3600 seconds")
...
def verify_authorization(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or value.get("schema") != AUTH_SCHEMA:
        raise ChainError("unsupported V8 payment authorization")
    ...
    if not isinstance(raw, Mapping):
        raise ChainError("V8 payment authorization is missing")
    authorization = PaymentAuthorization(
    ...
    if authorization.issued_at > current + 30 or authorization.deadline < current:
        raise ChainError("V8 payment authorization is outside its time window")
    if authorization.deadline <= authorization.issued_at or authorization.deadline - authorization.issued_at > MAX_AUTHORIZATION_TTL:
        raise ChainError("V8 payment authorization lifetime is invalid")
    ...
    if normalize_bytes32(str(value.get("authorization_hash") or "")) != struct_hash:
        raise ChainError("V8 authorization hash mismatch")
    ...
    if normalize_bytes32(str(value.get("authorization_digest") or "")) != "0x" + digest.hex():
        raise ChainError("V8 authorization digest mismatch")
    ...
    if recover_evm_address(digest, _evm_signature(signature)) != authorization.key:
        raise ChainError("V8 payment key signature mismatch")
...
def finalize_relay_receipt(value: Mapping[str, Any], *, relay_private_key: str) -> dict[str, Any]:
    ...
    if relay != relay_signer:
        raise ChainError("Relay key does not match V8 authorization relay signer")
    digest = receipt_digest(receipt, chain_id=chain_id, verifying_contract=contract)
...
def verify_provider_receipt(value: Mapping[str, Any]) -> tuple[dict[str, Any], UsageReceipt, bytes, int, str]:
    ...
    if int(value.get("chain_id") or 0) != chain_id or normalize_address(str(value.get("settlement_contract") or "")) != contract:
        raise ChainError("V8 Provider receipt deployment mismatch")
    ...
    if receipt.authorization_hash != authorization_struct_hash(_authorization_from_payload(authorization["authorization"])):
        raise ChainError("V8 Provider receipt authorization mismatch")
    digest = receipt_digest(receipt, chain_id=chain_id, verifying_contract=contract)
    if normalize_bytes32(str(value.get("receipt_digest") or "")) != "0x" + digest.hex():
        raise ChainError("V8 Provider receipt digest mismatch")
    ...
    if recover_evm_address(digest, _evm_signature(signature)) != receipt.provider_signer:
        raise ChainError("V8 Provider signature mismatch")
...
def encode_signed_batch(values: Sequence[Mapping[str, Any]]) -> str:
    ...
    if not tuples or len(tuples) > 32:
        raise ChainError("V8 batch must contain between 1 and 32 receipts")
...
def verify_signed_receipt(
    ...
    if receipt.authorization_hash != authorization_struct_hash(auth_struct):
        raise ChainError("V8 signed receipt authorization mismatch")
    if receipt.relay != normalize_address(str(auth_struct.relay)):
        raise ChainError("V8 signed receipt Relay payout mismatch")
    ...
    if key_signature != _raw_signature(authorization.get("key_signature"), "payment key"):
        raise ChainError("V8 signed receipt payment signature mismatch")
    ...
    if recover_evm_address(digest, _evm_signature(relay_signature)) != auth_struct.relay_signer:
        raise ChainError("V8 Relay signature mismatch")
...
def provider_signer_authorized(
    rpc_url: str,
    settlement: str,
    provider: str,
    signer: str,
    *,
    ...
    output = call_contract(
        rpc_url,
        settlement,
        "providerSigners(address,address)",
        [normalize_address(provider), normalize_address(signer)],
        timeout=timeout,
    )
    return int(_words(output, 1, "provider signer authorization")[0], 16) != 0


def authorize_provider_signer(
    *,
    rpc_url: str,
    provider_private_key: str,
    settlement: str,
    ...
    return send_contract_transaction(
        ...
        signature="authorizeProviderSigner(address)",
        args=[normalize_address(signer)],
        timeout=timeout,
    )


def revoke_provider_signer(
    *,
    rpc_url: str,
    provider_private_key: str,
    settlement: str,
    ...
    return send_contract_transaction(
        ...
        chain_id=chain_id,
        contract=normalize_address(settlement),
        signature="revokeProviderSigner(address)",
        args=[normalize_address(signer)],
        timeout=timeout,
    )
...
def claim_payout(
    *,
    ...
    return send_contract_transaction(
        rpc_url=rpc_url,
        private_key=payout_private_key,
        chain_id=chain_id,
        contract=normalize_address(settlement),
        signature="claim()",
        args=[],
...
def register_payment_key(
    *,
    ...
    """Register or refresh a reusable V8 bearer key from its wallet owner."""
    return send_contract_transaction(
        rpc_url=rpc_url,
...
def revoke_payment_key(
    *,
    ...
    """Revoke a reusable V8 bearer key from its wallet owner."""
    return send_contract_transaction(
        rpc_url=rpc_url,
        private_key=owner_private_key,
        chain_id=chain_id,
...
def deploy_testnet(
    *,
    ...
    if not artifact_path.exists():
        raise ChainError("V8 artifact is missing; run python3 scripts/compile-v8-artifact.py")
...
def save_deployment(path: Path, deployment: V8Deployment) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(deployment.to_dict(), indent=2, sort_keys=True) + "\n", encoding="utf-8")


def load_deployment(path: Path = Path(DEFAULT_DEPLOYMENT)) -> V8Deployment:
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict) or int(payload.get("protocol_version") or 0) != 8:
        raise ChainError("deployment is not Myco Settlement V8")
    names = {field.name for field in V8Deployment.__dataclass_fields__.values()}
    return V8Deployment(**{name: payload[name] for name in names if name in payload})
...
def _read_pricing_hash(rpc_url: str, settlement: str, channel: str, version: int, *, timeout: float) -> str:
    ...
    for attempt in range(3):
        try:
            ...
            if attempt < 2:
                time.sleep(1)
    raise last_error or ChainError("channel version lookup failed")
...
def _receipt_from_payload(raw: Any) -> UsageReceipt:
    if not isinstance(raw, Mapping):
        raise ChainError("V8 usage receipt is missing")
    return UsageReceipt(
...
def _expect(expected: Any | None, actual: Any, label: str) -> None:
    if expected is not None and int(expected) != int(actual):
        raise ChainError(f"V8 {label} mismatch")
