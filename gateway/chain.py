...
def load_active_myco_deployment(
    ...
    *,
    ...
    if version == 6:
        ...
        resolved = Path(
            path
            or values.get("MYCO_DEPLOYMENT")
            or values.get("MYCOMESH_SESSION_DEPLOYMENT")
            or DEFAULT_MYCO_V6_DEPLOYMENT_PATH
        )
        if not resolved.is_absolute() and not resolved.exists():
            bundled = Path(__file__).resolve().parent.parent / resolved
            if bundled.exists():
                resolved = bundled
        deployment = load_v6_deployment(resolved)
        configured_contract = str(
            values.get("MYCOMESH_SESSION_SETTLEMENT_CONTRACT")
            or values.get("MYCOMESH_SETTLEMENT_CONTRACT")
            or ""
        ).strip()
        if configured_contract and normalize_address(configured_contract) != deployment.settlement:
            raise ChainError("configured Settlement V6 address does not match the deployment manifest")
        configured_chain = str(
            values.get("MYCOMESH_SESSION_CHAIN_ID")
            or values.get("MYCOMESH_SETTLEMENT_CHAIN_ID")
            or ""
        ).strip()
        if configured_chain and int(configured_chain) != deployment.chain_id:
            raise ChainError("configured Settlement V6 chain id does not match the deployment manifest")
        return deployment

    if version == 5:
        from .chain_v5 import DEFAULT_MYCO_V5_DEPLOYMENT_PATH, load_deployment as load_v5_deployment

        resolved = Path(
            path
            or values.get("MYCO_DEPLOYMENT")
            or values.get("MYCOMESH_SESSION_DEPLOYMENT")
            or DEFAULT_MYCO_V5_DEPLOYMENT_PATH
        )
        if not resolved.is_absolute() and not resolved.exists():
            bundled = Path(__file__).resolve().parent.parent / resolved
            if bundled.exists():
                resolved = bundled
        deployment = load_v5_deployment(resolved)
        configured_contract = str(
            values.get("MYCOMESH_SESSION_SETTLEMENT_CONTRACT")
            or values.get("MYCOMESH_SETTLEMENT_CONTRACT")
            or ""
        ...
        if configured_contract and normalize_address(configured_contract) != deployment.settlement:
            raise ChainError("configured Settlement V5 address does not match the deployment manifest")
        configured_chain = str(
            values.get("MYCOMESH_SESSION_CHAIN_ID")
            or values.get("MYCOMESH_SETTLEMENT_CHAIN_ID")
            or ""
        ...
        if configured_chain and int(configured_chain) != deployment.chain_id:
            raise ChainError("configured Settlement V5 chain id does not match the deployment manifest")
        return deployment

    if version == 4:
        from .chain_v4 import DEFAULT_MYCO_V4_DEPLOYMENT_PATH, load_deployment as load_v4_deployment

        resolved = Path(path or values.get("MYCO_DEPLOYMENT") or DEFAULT_MYCO_V4_DEPLOYMENT_PATH)
        if not resolved.is_absolute() and not resolved.exists():
            bundled = Path(__file__).resolve().parent.parent / resolved
            if bundled.exists():
                resolved = bundled
        deployment = load_v4_deployment(resolved)
        configured_contract = str(
            values.get("MYCOMESH_SESSION_SETTLEMENT_CONTRACT")
            or values.get("MYCOMESH_SETTLEMENT_CONTRACT")
            or ""
        ...
        if configured_contract and normalize_address(configured_contract) != deployment.settlement:
            raise ChainError("configured Settlement V4 address does not match the deployment manifest")
        configured_chain = str(values.get("MYCOMESH_SESSION_CHAIN_ID") or values.get("MYCOMESH_SETTLEMENT_CHAIN_ID") or "").strip()
        if configured_chain and int(configured_chain) != deployment.chain_id:
            raise ChainError("configured Settlement V4 chain id does not match the deployment manifest")
        return deployment
