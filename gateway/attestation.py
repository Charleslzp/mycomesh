    ...
    if settlement_version in {3, 4, 5, 6}:
        ...
        if settlement_version in {4, 5, 6}:
            ...
            if not BYTES32_PATTERN.fullmatch(session_id) or int(session_id[2:], 16) == 0:
                raise AttestationError(f"Settlement V{settlement_version} provider attestation requires session_id")
            sequence = _integer(document.get("session_sequence"), "session_sequence")
            if sequence <= 0 or sequence > UINT256_MAX:
                raise AttestationError(f"Settlement V{settlement_version} provider attestation requires session_sequence")
            ...
            if not BYTES32_PATTERN.fullmatch(authorization_hash) or int(authorization_hash[2:], 16) == 0:
                raise AttestationError(f"Settlement V{settlement_version} provider attestation requires authorization_hash")
            amount_units = _integer(document.get("amount_units"), "amount_units")
            if amount_units < 0 or amount_units > UINT256_MAX:
                raise AttestationError(f"Settlement V{settlement_version} provider attestation amount_units is out of range")
            if amount_units != _integer(document.get("gross_fee_units"), "gross_fee_units"):
                raise AttestationError(f"Settlement V{settlement_version} provider attestation amount_units mismatch")
        if deadline <= 0:
