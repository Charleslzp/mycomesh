...
"""Settlement V5 helpers for immutable routes and Relay attestations."""
...
MYCO_V5_DOMAIN_TYPE = "EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)"
MYCO_V5_SESSION_RECEIPT_TYPE = (
...
MYCO_V5_RELAY_ATTESTATION_TYPE = (
    "RelayRequestAttestation(bytes32 sessionId,bytes32 requestHash,address provider,address relay,"
    "uint256 sequence,uint256 deadline)"
)
MYCO_V5_DEPLOYER_ARTIFACT = "out/MycoSettlementV5.sol/MycoSettlementV5.json"
DEFAULT_MYCO_V5_DEPLOYMENT_PATH = "deployments/sepolia-myco-v5.json"
...
V5_RELAY_ATTESTATION_SCHEMA = "mycomesh.relay.v5.attestation.v1"
...
def domain_separator(*, chain_id: int, verifying_contract: str) -> str:
    encoded = b"".join(
        [
            keccak256(MYCO_V5_DOMAIN_TYPE.encode("utf-8")),
...
def session_receipt_typehash() -> str:
    return "0x" + keccak256(MYCO_V5_SESSION_RECEIPT_TYPE.encode("utf-8")).hex()


def relay_attestation_typehash() -> str:
    return "0x" + keccak256(MYCO_V5_RELAY_ATTESTATION_TYPE.encode("utf-8")).hex()


def session_receipt_struct_hash(receipt: V5SessionReceipt) -> str:
    return "0x" + keccak256(
...
def session_receipt_digest(receipt: V5SessionReceipt, *, chain_id: int, verifying_contract: str) -> bytes:
...
def build_runtime_session_receipt(
    *,
    ...
    if values["consumer"] == values["provider"]:
        raise ChainError("V5 consumer and provider must differ")
    ...
    return V5SessionReceipt(receipt_hash=receipt_hash, accepted_hash=accepted_hash, **values)
...
def build_provider_settlement_payload(*, provider_private_key: str, chain_id: int, settlement_contract: str, **values: Any) -> dict[str, Any]:
    ...
    if signer != normalize_address(receipt.provider):
        raise ChainError("Provider EVM identity does not match V5 receipt provider")
...
def build_relay_attestation(
    *,
    ...
    return {
        "schema": V5_RELAY_ATTESTATION_SCHEMA,
...
def verify_relay_attestation(
    ...
    *,
    expected_signer: str,
    receipt: V5SessionReceipt | None = None,
    expected_chain_id: int | None = None,
    expected_contract: str | None = None,
    now: int | None = None,
) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != V5_RELAY_ATTESTATION_FIELDS:
        raise ChainError("Relay V5 attestation fields are invalid")
    if value.get("schema") != V5_RELAY_ATTESTATION_SCHEMA:
        raise ChainError("unsupported Relay V5 attestation schema")
    normalized = {
        "schema": V5_RELAY_ATTESTATION_SCHEMA,
    ...
    if normalized["signer"] != signer:
        raise ChainError("Relay V5 attestation signer does not match the session route")
    if expected_chain_id is not None:
        ...
        if normalized["chain_id"] != chain_id:
            raise ChainError("Relay V5 attestation chain_id does not match the deployment")
    if expected_contract is not None:
        contract = _nonzero_address(expected_contract, "expected_contract")
        if normalized["settlement_contract"] != contract:
            raise ChainError("Relay V5 attestation contract does not match the deployment")
    current = int(time.time() if now is None else now)
    if normalized["deadline"] < current:
        raise ChainError("Relay V5 attestation deadline has elapsed")
    digest = relay_attestation_digest(
    ...
    if normalized["attestation_digest"] != "0x" + digest.hex():
        raise ChainError("Relay V5 attestation digest mismatch")
    signature = _strict_signature(normalized["signature"], "Relay attestation")
    if recover_evm_address(digest, signature) != signer:
        raise ChainError("Relay V5 attestation signature does not recover the session signer")
    if receipt is not None:
        ...
        for field, expected_value in expected.items():
            ...
            if str(actual).lower() != str(expected_value).lower():
                raise ChainError(f"Relay V5 attestation {field} does not match the receipt")
    return normalized


def verify_provider_settlement_payload(payload: Any) -> V5SessionReceipt:
    if not isinstance(payload, dict) or set(payload) != V5_PAYLOAD_FIELDS:
        raise ChainError("Provider V5 settlement payload fields are invalid")
    if payload.get("schema") != V5_SETTLEMENT_SCHEMA:
        raise ChainError("unsupported Provider V5 settlement payload schema")
    ...
    if not isinstance(raw, dict) or set(raw) != V5_RECEIPT_FIELDS:
        raise ChainError("Provider V5 settlement receipt fields are invalid")
    ...
    if receipt.receipt_hash != expected.receipt_hash or receipt.accepted_hash != expected.accepted_hash:
        raise ChainError("Provider V5 settlement receipt commitment mismatch")
    ...
    if _nonzero_bytes32(payload.get("receipt_digest"), "receipt_digest") != "0x" + digest.hex():
        raise ChainError("Provider V5 settlement receipt_digest mismatch")
    if recover_evm_address(digest, _strict_signature(payload.get("provider_signature"), "provider")) != receipt.provider:
        raise ChainError("Provider V5 settlement signature does not recover receipt provider")
    return receipt
...
def encode_settle_signed_receipt(
    ...
    if relay_attestation is None:
        attestation_values = ["0x" + "0" * 64, "0x" + "0" * 64, ZERO_ADDRESS, ZERO_ADDRESS, "0", "0"]
    else:
        attestation_values = [
...
def encode_claim_payout() -> str:
    return "0x" + keccak256(V5_CLAIM_PAYOUT_SIGNATURE.encode("utf-8"))[:4].hex()
...
def deploy_testnet(
    *,
    ...
    artifact: str = MYCO_V5_DEPLOYER_ARTIFACT,
    timeout: float = 300.0,
) -> V5Deployment:
    if not Path(artifact).exists():
        try:
            ...
            try:
                run_tool(["python3", "scripts/compile-v5-artifact.py"], timeout=timeout)
            except ChainError as compile_error:
                raise ChainError(
                    f"V5 artifact is missing; forge build failed ({forge_error}) and solc fallback failed ({compile_error})"
                ) from compile_error
...
def save_deployment(path: Path, deployment: V5Deployment) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(deployment.to_dict(), indent=2, sort_keys=True) + "\n", encoding="utf-8")


def load_deployment(path: Path = Path(DEFAULT_MYCO_V5_DEPLOYMENT_PATH)) -> V5Deployment:
    if not path.exists():
        raise ChainError(f"Myco V5 deployment not found: {path}")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict) or int(payload.get("protocol_version") or 0) != 5:
        raise ChainError("deployment is not a Myco Settlement V5 deployment")
    required = {
    ...
    if missing:
        raise ChainError("Myco V5 deployment is missing required fields: " + ", ".join(missing))
    return V5Deployment(
        protocol_version=5,
        ...
        eip712_version=str(payload.get("eip712_version") or "5"),
...
def _runtime_receipt_hash(values: Mapping[str, Any]) -> str:
    commitment = (
        "MycoMeshV5SessionReceipt(bytes32 sessionId,bytes32 requestHash,bytes32 responseHash,bytes32 channel,"
    ...
    order = (
        ...
        "provider", "relay", "pool", "input_tokens", "output_tokens", "sequence", "quoted_fee", "deadline",
    )
...
def _runtime_acceptance_hash(*, receipt_hash: str, session_id: str, consumer: str, provider: str) -> str:
    commitment = "MycoMeshV5ConsumerAcceptance(bytes32 receiptHash,bytes32 sessionId,address consumer,address provider)"
    encoded = b"".join(
...
def _validate_artifact(path: Path) -> None:
    try:
        ...
        raise ChainError(f"could not read V5 artifact {path}: {exc}") from exc
    abi = payload.get("abi") if isinstance(payload, dict) else None
    if not isinstance(abi, list):
        raise ChainError(f"V5 artifact does not contain an ABI: {path}")
    ...
    if missing:
        raise ChainError("V5 artifact ABI is missing required functions: " + ", ".join(missing))
