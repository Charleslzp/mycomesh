#!/usr/bin/env python3
"""Run mandatory V10 EVM tests with locked, temporary, loopback-only fixtures.
...
TEST_CLASSES = (
    "tests.test_v10_local_chain.V10LocalChainTests",
)
MINIMUM_TESTS = 8
...
def loopback_only(event: str, args: tuple) -> None:
    ...
    try:
        ...
        raise RuntimeError("V10 integration tests permit only loopback RPC") from exc
    if not address.is_loopback:
        raise RuntimeError("V10 integration tests permit only loopback RPC")
...
def run_suite() -> int:
    ...
    if selected < MINIMUM_TESTS:
        raise RuntimeError(f"Expected at least {MINIMUM_TESTS} V10 EVM tests; found {selected}")
    result = unittest.TextTestRunner(verbosity=2).run(suite)
    if result.skipped:
        print(f"ERROR: skipped V10 EVM tests are forbidden: {result.skipped}", file=sys.stderr)
    if result.testsRun != selected:
        print(f"ERROR: executed {result.testsRun} of {selected} V10 EVM tests", file=sys.stderr)
...
def main() -> int:
    ...
    if node_version != "v22.22.2":
        raise RuntimeError(f"V10 EVM tests require pinned Node v22.22.2; found {node_version}")
    ...
    with tempfile.TemporaryDirectory(prefix="mycomesh-v10-local-chain-") as directory:
