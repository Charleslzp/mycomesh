#!/usr/bin/env python3
"""Compile the standalone V5 contract into a Foundry-compatible artifact."""
...
SOURCE = ROOT / "contracts" / "MycoSettlementV5.sol"
ARTIFACT = ROOT / "out" / "MycoSettlementV5.sol" / "MycoSettlementV5.json"
...
def main() -> int:
    ...
    def compile_outputs(outputs: list[str]) -> dict:
        request = {
            ...
            "settings": {
                ...
                "outputSelection": {"*": {"MycoSettlementV5": outputs}},
            },
        }
        try:
            ...
            raise RuntimeError(f"V5 Solidity compiler failed to start: {exc}") from exc
        ...
        contract = output.get("contracts", {}).get(SOURCE.name, {}).get("MycoSettlementV5")
        if not isinstance(contract, dict):
            raise RuntimeError("solc output does not contain MycoSettlementV5")
        return contract
    ...
    if (
        ...
        print("solc output is missing V5 ABI or bytecode", file=sys.stderr)
        return 1
