...
class ProviderV7Test(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def message(self, config: ProviderConfig) -> dict[str, object]:
        ...
        unsigned["payment_v7"] = build_authorization(
    ...
    def test_preverify_has_no_session_state(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(reservation["settlement_version"], 7)
            ...
            self.assertTrue(checked["request_key"].startswith("v7:"))

    def test_v7_replay_key_is_independent_of_relay_scheduler_identity(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_inference_returns_independent_provider_receipt(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            authorization, receipt, _, _, _ = verify_provider_receipt(response["mycomesh_v7_settlement"])
    ...
    def test_same_v7_request_replays_cached_result_without_second_upstream_call(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            config = self.config(directory)
    ...
    def test_same_request_can_refresh_cached_receipt_for_a_new_relay(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                raw_payment = unsigned["payment_v7"]["authorization"]
                unsigned["payment_v7"] = build_authorization(
                    payment_key=self.payment_key,
                    chain_id=int(config.settlement_chain_id or 0),
                    settlement_contract=str(config.settlement_contract),
            ...
            self.assertTrue(first["ok"])
            self.assertTrue(second["ok"])
            _, receipt, _, _, _ = verify_provider_receipt(second["mycomesh_v7_settlement"])
            self.assertEqual(receipt.relay, next_relay)
            self.assertEqual(upstream.call_count, 1)

    def test_explicit_upstream_rejection_releases_v7_execution_fence(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            config = self.config(directory)
            message = self.message(config)
            with (
