...
interface IMycoERC20V6 {
    function balanceOf(address account) external view returns (uint256);
    function transfer(address to, uint256 amount) external returns (bool);
    function transferFrom(address from, address to, uint256 amount) external returns (bool);
}

interface IMycoRewardTokenV6 {
    function mint(address to, uint256 amount) external;
}

/// @notice V6 session escrow for long-lived, prepaid inference sessions.
///
/// A consumer funds one session on-chain and binds its Provider and Pool.
/// Relay payout/signing routes are versioned by epoch so the Consumer can
/// rotate a failed Relay without invalidating receipts from the previous
/// route. Inference receipts are signed by the session EOA and Provider;
/// relayed requests also carry a request-scoped Relay signature. A valid
/// receipt can be submitted by any transaction sender in a batch. A session
/// is closed only after an explicit consumer
...
contract MycoSettlementV6 {
    ...
    bytes32 public constant SESSION_RECEIPT_TYPEHASH = keccak256(
        "SessionReceipt(bytes32 receiptHash,bytes32 acceptedHash,bytes32 sessionId,bytes32 requestHash,bytes32 responseHash,bytes32 channel,uint64 pricingVersion,bytes32 pricingHash,address consumer,address provider,address relay,address pool,uint64 relayEpoch,uint256 inputTokens,uint256 outputTokens,uint256 sequence,uint256 quotedFee,uint256 deadline)"
    );
    bytes32 public constant RELAY_REQUEST_ATTESTATION_TYPEHASH = keccak256(
        "RelayRequestAttestation(bytes32 sessionId,bytes32 requestHash,address provider,address relay,uint64 relayEpoch,uint256 sequence,uint256 deadline)"
    );
    ...
    struct RelayRequestAttestation {
        bytes32 sessionId;
        bytes32 requestHash;
        address provider;
        address relay;
    ...
    struct SignedSessionReceipt {
        ...
        RelayRequestAttestation relayAttestation;
    ...
    IMycoERC20V6 public immutable stablecoin;
    IMycoRewardTokenV6 public immutable rewardToken;
    ...
    mapping(bytes32 => mapping(uint64 => RelayRoute)) private sessionRoutes;
    ...
    event SessionRelayRotated(
        bytes32 indexed sessionId,
        uint64 indexed previousEpoch,
        uint64 indexed relayEpoch,
        address relay,
        address relaySigner
    );
    ...
    constructor(
        ...
        stablecoin = IMycoERC20V6(stablecoin_);
        rewardToken = IMycoRewardTokenV6(rewardToken_);
        treasury = treasury_;
    ...
    function openSession(
        ...
        require(relay != address(this) && pool != address(this), "invalid recipient");
        require((relay == address(0)) == (relaySigner == address(0)), "relay signer mismatch");
        require(relaySigner == address(0) || relaySigner.code.length == 0, "relay signer must be EOA");
        require(
            relaySigner == address(0)
                || (relaySigner != relay && relaySigner != provider && relaySigner != sessionKey && relaySigner != msg.sender),
            "bad relay signer"
        );
        ...
        sessionRoutes[sessionId][0] = RelayRoute({relay: relay, relaySigner: relaySigner});
        emit SessionOpened(
            sessionId,
            msg.sender,
            provider,
    ...
    /// @notice Rotate the active Relay route without moving Consumer funds.
    /// Previous epochs remain readable so receipts already in flight can
    /// still settle against the Relay that signed them.
    function rotateSessionRelay(bytes32 sessionId, address relay, address relaySigner)
        external
        nonReentrant
        returns (uint64 nextEpoch)
    {
        Session storage session = sessions[sessionId];
        require(session.consumer != address(0), "unknown session");
        require(msg.sender == session.consumer, "not consumer");
        ...
        require(
            relaySigner == address(0)
                || (relaySigner != relay && relaySigner != session.provider && relaySigner != session.sessionKey
                    && relaySigner != session.consumer),
            "bad relay signer"
        );
        require(session.relayEpoch < type(uint64).max, "relay epoch exhausted");
        uint64 previousEpoch = session.relayEpoch;
        nextEpoch = previousEpoch + 1;
        session.relayEpoch = nextEpoch;
        session.relay = relay;
        session.relaySigner = relaySigner;
        sessionRoutes[sessionId][nextEpoch] = RelayRoute({relay: relay, relaySigner: relaySigner});
        emit SessionRelayRotated(sessionId, previousEpoch, nextEpoch, relay, relaySigner);
    }

    function currentRelayEpoch(bytes32 sessionId) external view returns (uint64) {
        return sessions[sessionId].relayEpoch;
    }

    function sessionRoute(bytes32 sessionId, uint64 relayEpoch)
        external
        view
        returns (address relay, address relaySigner)
    {
        RelayRoute storage route = sessionRoutes[sessionId][relayEpoch];
        return (route.relay, route.relaySigner);
    }
    ...
    function relayRequestAttestationStructHash(RelayRequestAttestation calldata attestation)
        public
        pure
        returns (bytes32)
    {
        return keccak256(abi.encode(RELAY_REQUEST_ATTESTATION_TYPEHASH, attestation));
    }

    function relayRequestAttestationDigest(RelayRequestAttestation calldata attestation)
        public
        view
        returns (bytes32)
    {
        return _toTypedDataHash(relayRequestAttestationStructHash(attestation));
    }
    ...
    function _settleSignedReceipt(SignedSessionReceipt calldata input) internal {
        ...
        _validateRelayAttestation(input, session);
        _settle(input.receipt, grossFee);
    }

    function _validateRelayAttestation(SignedSessionReceipt calldata input, Session storage session) internal view {
        RelayRequestAttestation calldata attestation = input.relayAttestation;
        require(input.receipt.relayEpoch <= session.relayEpoch, "unknown relay epoch");
        RelayRoute storage route = sessionRoutes[input.receipt.sessionId][input.receipt.relayEpoch];
        if (route.relay == address(0)) {
            require(
                input.relaySignature.length == 0 && attestation.sessionId == bytes32(0)
                    && attestation.requestHash == bytes32(0)
                    && attestation.provider == address(0) && attestation.relay == address(0)
                    && attestation.relayEpoch == 0
                    && attestation.sequence == 0 && attestation.deadline == 0,
                "unexpected relay proof"
            );
            return;
        }

        require(input.relaySignature.length != 0, "missing relay signature");
        require(
            attestation.sessionId == input.receipt.sessionId && attestation.requestHash == input.receipt.requestHash
                && attestation.provider == session.provider && attestation.relay == route.relay
                && attestation.relayEpoch == input.receipt.relayEpoch
                && attestation.sequence == input.receipt.sequence && attestation.deadline == input.receipt.deadline,
            "relay attestation mismatch"
        );
        bytes32 relayDigest = _toTypedDataHash(relayRequestAttestationStructHash(attestation));
        require(_recover(relayDigest, input.relaySignature) == route.relaySigner, "bad relay signature");
    }
    ...
    function _validateReceipt(SessionReceipt calldata receipt) internal view returns (uint256 grossFee) {
        ...
        require(receipt.relayEpoch <= session.relayEpoch, "unknown relay epoch");
        RelayRoute storage route = sessionRoutes[receipt.sessionId][receipt.relayEpoch];
        require(route.relay == receipt.relay, "session relay");
        require(session.pool == receipt.pool, "session pool");
        require(
            session.closeRequestedAt == 0
                || block.timestamp < uint256(session.closeRequestedAt) + CLOSE_GRACE_SECONDS,
            "close grace elapsed"
    ...
    function _tokenBalance(address account) internal view returns (uint256 balance) {
        (bool ok, bytes memory data) = address(stablecoin).staticcall(abi.encodeCall(IMycoERC20V6.balanceOf, (account)));
    ...
    function _safeTransfer(address to, uint256 amount) internal {
        ...
        _callOptionalReturn(abi.encodeCall(IMycoERC20V6.transfer, (to, amount)));
    ...
    function _safeTransferFrom(address from, address to, uint256 amount) internal {
        ...
        uint256 beforeTo = _tokenBalance(to);
        _callOptionalReturn(abi.encodeCall(IMycoERC20V6.transferFrom, (from, to, amount)));
