...
interface IMycoERC20V7 {
    function balanceOf(address account) external view returns (uint256);
    function transfer(address to, uint256 amount) external returns (bool);
    function transferFrom(address from, address to, uint256 amount) external returns (bool);
}

/// @notice Stateless prepaid settlement for MycoMesh inference requests.
/// @dev Wallets register reusable payment keys. Each request is independently
/// authorized, so Provider and Relay routing never becomes account state.
contract MycoSettlementV7 {
    ...
    bytes32 public constant USAGE_RECEIPT_TYPEHASH = keccak256(
        "UsageReceipt(bytes32 authorizationHash,bytes32 responseHash,address provider,address relay,address pool,uint256 inputTokens,uint256 outputTokens,uint256 actualFee)"
    );
    ...
    IMycoERC20V7 public immutable stablecoin;
    ...
    constructor(
        ...
        stablecoin = IMycoERC20V7(stablecoin_);
        treasury = treasury_;
    ...
    function _settle(SignedReceipt calldata input) internal {
        ...
        require(_recover(usageDigest, input.providerSignature) == receipt.provider, "bad provider signature");
    ...
    function _safeTransfer(address to, uint256 amount) internal {
        (bool ok, bytes memory data) = address(stablecoin).call(
            abi.encodeWithSelector(IMycoERC20V7.transfer.selector, to, amount)
        );
    ...
    function _safeTransferFrom(address from, address to, uint256 amount) internal {
        (bool ok, bytes memory data) = address(stablecoin).call(
            abi.encodeWithSelector(IMycoERC20V7.transferFrom.selector, from, to, amount)
        );
