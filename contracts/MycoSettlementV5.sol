...
interface IMycoERC20V5 {
    function balanceOf(address account) external view returns (uint256);
    function transfer(address to, uint256 amount) external returns (bool);
    function transferFrom(address from, address to, uint256 amount) external returns (bool);
}

interface IMycoRewardTokenV5 {
    function mint(address to, uint256 amount) external;
}

/// @notice V5 session escrow for long-lived, prepaid inference sessions.
///
/// A consumer funds one session on-chain and fixes its Provider, Relay payout,
/// Relay signer and Pool. Inference receipts are signed by the session EOA and
/// Provider; relayed requests also carry a request-scoped Relay signature. A
/// valid receipt can be submitted by any transaction sender in a batch. A
/// session is closed only after an explicit consumer
...
contract MycoSettlementV5 {
    ...
    bytes32 public constant RELAY_REQUEST_ATTESTATION_TYPEHASH = keccak256(
        "RelayRequestAttestation(bytes32 sessionId,bytes32 requestHash,address provider,address relay,uint256 sequence,uint256 deadline)"
    );
    ...
    IMycoERC20V5 public immutable stablecoin;
    IMycoRewardTokenV5 public immutable rewardToken;
    ...
    constructor(
        ...
        stablecoin = IMycoERC20V5(stablecoin_);
        rewardToken = IMycoRewardTokenV5(rewardToken_);
        treasury = treasury_;
    ...
    function _validateRelayAttestation(SignedSessionReceipt calldata input, Session storage session) internal view {
        ...
        require(
            attestation.sessionId == input.receipt.sessionId && attestation.requestHash == input.receipt.requestHash
                && attestation.provider == session.provider && attestation.relay == session.relay
        ...
        require(_recover(relayDigest, input.relaySignature) == session.relaySigner, "bad relay signature");
    }
    ...
    function _validateReceipt(SessionReceipt calldata receipt) internal view returns (uint256 grossFee) {
        ...
        require(session.relay == receipt.relay, "session relay");
    ...
    function _tokenBalance(address account) internal view returns (uint256 balance) {
        (bool ok, bytes memory data) = address(stablecoin).staticcall(abi.encodeCall(IMycoERC20V5.balanceOf, (account)));
    ...
    function _safeTransfer(address to, uint256 amount) internal {
        ...
        _callOptionalReturn(abi.encodeCall(IMycoERC20V5.transfer, (to, amount)));
    ...
    function _safeTransferFrom(address from, address to, uint256 amount) internal {
        ...
        uint256 beforeTo = _tokenBalance(to);
        _callOptionalReturn(abi.encodeCall(IMycoERC20V5.transferFrom, (from, to, amount)));
