...
| Public Node | Docker: separate Bridge and Relay containers on the official host | Yes | Node identities; never Codex auth |
...
Bridge and Relay are one official public-node operator role but remain separate
logical services and containers. Bridge never carries inference payloads. Relay
forwards bounded, replay-protected sealed frames, signs request-level online
attestations, and never parses prompts, OAuth tokens, OpenAI payloads, usage, or
billing data. Running an API on a Relay host means running a separate Provider;
it does not change the Relay role.
