...
| OpenAI error envelope and status | Supported | Provider HTTP status and error objects survive P2P/Relay transport; HTTP errors use `error.message/type/param/code`, and WebSocket errors use the Responses `error` event shape. Retryable upstream status can fail over to another Provider. |
