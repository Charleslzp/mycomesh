...
.PHONY: deploy-env build gateway proxy bridge relay provider provider-identity proxy-identity demo up down logs ps test smoke package-install
...
provider-identity: deploy-env
	$(COMPOSE) --env-file .env.deploy --profile provider run --rm --entrypoint python provider -m gateway identity show --identity /data/node-identity.json
