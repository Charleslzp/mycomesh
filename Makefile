...
PUBLIC_NODE_ENV = \
	...
	MYCOMESH_BRIDGE_SETTLEMENT_VERSION=$(PUBLIC_NODE_SETTLEMENT_VERSION) \
	MYCOMESH_BRIDGE_DEPLOYMENT=$(PUBLIC_NODE_DEPLOYMENT) \
	...
	MYCOMESH_RELAY_SETTLEMENT_VERSION=$(PUBLIC_NODE_SETTLEMENT_VERSION) \
	MYCOMESH_RELAY_NETWORK_CONFIG=$(PUBLIC_NODE_NETWORK_CONFIG) \
...
.PHONY: deploy-env proxy-configure proxy-preflight proxy-relayer-address relay-transaction-address require-node-image require-provider-image build images-show node-image-pull provider-image-pull images-pull consumer consumer-up consumer-up-image consumer-open consumer-codex consumer-down consumer-health consumer-logs consumer-credentials consumer-codex-env consumer-cli-test gateway proxy proxy-up proxy-up-image proxy-down proxy-health proxy-logs proxy-identity proxy-identity-import bridge relay relay-up relay-down relay-onboard relay-start node-up node-health public-node-up public-node-up-image main-node-up-image public-node-down public-node-health public-node-tls-health public-node-logs provider provider-login provider-login-image provider-authorize provider-operator-config-export-image provider-identity-export-image provider-config-apply-image provider-auth-reset-image provider-auth-ensure-image provider-auth-status-image provider-up provider-up-image provider-configure provider-onboard provider-start provider-down provider-health provider-logs provider-identity provider-claim-payout release-check demo up down logs ps test smoke package-install web-install nginx-bootstrap-install nginx-install
